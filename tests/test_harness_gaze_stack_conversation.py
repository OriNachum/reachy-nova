...
def conversation_stack(intents, attention=None, clock=None, lock_state=None):
...
def test_conversation_looks_at_the_sound_then_locks_the_face():
    ...
    try:
        ...
        # One tick, two ops, in that order and no others.
        assert intents.since(mark) == ["look_at_sound", "lock_face"]
...
def test_the_lock_is_held_across_a_quiet_gap_and_released_at_fade():
    ...
    try:
        ...
        # gaps are inside the window, so nothing is submitted.
        clock.advance(10.0)
        time.sleep(TICK * 8)
