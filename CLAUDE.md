...
- **Integration**: `nova_context.py` (`NovaContext`, the DI container passed to skill executors/API routes), `skill_executors.py` (the tool-callable implementations behind Sonic's `toolConfiguration`), `api_routes.py` (the FastAPI dashboard endpoints under `/api/*`), `nova_mqtt.py` (the MQTT nervous-system bridge — publishes `nova/events/<source>/<type>`, subscribes to inject commands; every publishing source has an explicit priority/urgency rule in `config/nervous-system/rules.yaml`), `harness/lite_reactor.py` (`LiteReactor`, Nova 2 Lite one-line reaction plans for `react: lite` rules.yaml entries, on their own worker with a timeout + template fallback — see `docs/components/lite-reactor.md`), `harness/switches.py` (`Switches`, the round's env kill-switches — `NOVA_CHUNKED_PLAYBACK`/`NOVA_LITE_REACTIONS`/`NOVA_MEMORY`/`NOVA_FACE_HOLD`/`NOVA_THINK_POSTURE`/`NOVA_ATTENTION_GATE`/`NOVA_PERSONA_PATH` — resolved once and logged at start).
- **Harness gaze/voice**: `harness/lock_state.py` (`LockState`, the harness's own belief about a runtime gaze lock — owner `auto`/`model` — updated from confirmed `lock_face`/`release_face` results and `motion/lock-released` events — see `docs/components/gaze.md`), `harness/gaze_stack.py` (`GazeStack`, the single-writer posture layer — `wander < browsing < conversation`, a standing `gaze-hold` goal while browsing, `look_at_sound` + auto-owned `lock_face` while a conversation is live, `clear_for_result()` before a browse result, and a bounded passive `run_behavior feel-alive` re-issued whenever the runtime's active list lost its base layer after a lock (reachy-mini-cli #183) — see `docs/components/gaze.md`; `harness/tools.py`'s `think` alias tool runs the same "thoughtful" runtime behaviour one-shot, side left/right alternating, next to `look_at_face`/`look_at_sound`), `harness/attention.py` (`AttentionState`, the cold/warm window a spoken "Nova"/"Reachy" opens — gates the voice at the speaker, gates the gaze via `conversation_live`, and gates the effectful tools in `tools.py` (`COLD_REFUSED_TOOLS`) while cold and nameless — see `docs/components/attention.md`), `harness/eyes.py` (`Eyes`/`EyesComponent`, a ~1 Hz watch on the runtime's camera-availability snapshot that latches one dropped/restored line per transition, since the runtime's own availability report reflects composition, not liveness — see `docs/components/eyes.md`), `harness/quiet.py` (`QuietState`, the persisted, self-expiring timed-quiet deadline behind `stay_silent`/`end_silence` — see `docs/components/quiet-mode.md`).
...
- Feature switches (harness, fail-open): `NOVA_CHUNKED_PLAYBACK`, `NOVA_LITE_REACTIONS`, `NOVA_MEMORY`, `NOVA_FACE_HOLD`, `NOVA_THINK_POSTURE`, `NOVA_ATTENTION_GATE`, `NOVA_PERSONA_PATH` — resolved once and logged at start (`harness/switches.py`); see `.env.sample`
