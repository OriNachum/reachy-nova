...
produced five more harness fixes and one prerequisite for the runtime's
...
| `t7` | delivered | `think` alias, typed duplicate passthrough, model-owned locks, cold refusal of effectful tools; `49e2596` → `49979fb`; live fix `af9276c` (`d9`): the refusal gates the model's tool calls only — the gaze stack's ops pass `reflex=True` |
| `t8` | delivered | `harness/gaze_stack.py` core; `cb75099` → `5d92414`; review: start hygiene moved onto the worker (`0d6c577` → `6619e82`) |
| `t9` | delivered | conversation layer with backoff, owner, fade release, start/stop hygiene; `cf04c47` → `6a9d652`; live fix `61bde6f` (`d5`): the layer opens only on someone else speaking, and `antenna-sway` (amp 10, period 5, re-issued) keeps the antennas alive under the automatic lock (`lock_liveness`); live fix `9aab323` (`d8`): the runtime's own `feel-alive` base layer, evicted by every lock and never re-seeded (reachy-mini-cli #183), is revived by the stack as a bounded passive `run_behavior` whenever the active list lacks it |
...
| `t13` | delivered | docs (attention, gaze, eyes, browser, sonic, architecture, CLAUDE.md), version 0.5.0; `0334397` → `d5f044b`; plus `67d5d75` (names-table paragraph in attention.md, architecture 6.3), `9aab323` (base-layer revive in gaze.md, CLAUDE.md) and `af9276c` (reflex bypass in attention.md, gaze.md) |
| `t14` | delivered | audit at `8827b56` and re-run at `6619e82`: boundary 19, no legacy imports, act path unchanged, daemon paths unchanged; suite 2117 at `af9276c` |
| `t15` | partial | "nova" reached the runtime's name lists in the first form of reachy-mini-cli PR #178 (`a8535a2` → `b3fd068`), then the operator ruled on #175 that the runtime hardcodes no peer's name: the commit was dropped from #178 and reverted on the device branch. The runtime agent shipped the configurable `names` overlay table plus the `name_mentioned` sense field as reachy-mini-cli #182 (v0.53.0, main `0817403`); its prerequisite on this repo (#27) landed as `67d5d75` — the overlay validator accepts both (`d6`). The robot's device branch `wireless-motor-enable` is still `cb1ab7c` (0.52.1), so the harness's own `names = ["nova"]` write waits until that runtime is on the robot |
...
| `t18` | delivered | robot moved to main (`19dde0c`, 08:25Z), then the round branch (`d5f044b` 08:31Z → `6619e82` → `ed198f5` → `61bde6f` 10:18Z → `67d5d75` 10:43Z → `9aab323` 11:05Z → `af9276c` 11:43Z); switches and persona lines read from the journal at every step |
...
| `t20` | partial | item 2 proven live with Ori in view: 10:48–10:53 local lock 3.3 s after entry, held 42 s, released on fade, re-locked in 0.3 s; 11:30:33 local "noah look at me" → window opened by name, `look_at_sound`, lock confirmed on the first attempt 0.2 s later, antennas swaying under the lock, released on fade; cold nameless speech ("oh", "hello hello") dropped with no audio. Operator verdicts: "look at me works, but very slow, and not my face on center", "It feels rigid now. No liveness." (fixed, `d5`), "It follows me in a delay", "not focusing on me on center of the camera" — both the runtime's aim (reachy-mini-cli #181) and detection cadence (#179/#180); "Is reachy deployed? I don't see it moving" (11:51) — the runtime's base layer evicted by the lock and never re-seeded (#183), revived by the harness from `9aab323` (`d8`, proven live 12:06: evicted by name, back in 2 s, head and antennas moving); then, 12:28–12:36, a nameless Hebrew conversation at the desk had the attention gate refuse every lock attempt and every revive as "not addressed" (`d9`, fixed `af9276c`, proven live 12:43: revive ok, the next nameless sentence's lock reached the runtime). Still not run: the spoken browse (once, then deduped), the five-by-five name count, the browsing posture, a guest, the tone verdict |

## Mid-work Decisions

All nine deviation records are **proposed** (`--origin llm`) and await Ori's
...
- `d8` — Ori, 11:51 BST: "Is reachy deployed? I don't see it moving." The
  ...
  base layer once the inhibition clears (`engine.py` seeds it once at start;
  `_sustain_goal` re-admits goals only) — after the first lock of a session
  the robot is still for good (`state.json` `active: []`, head parked). Filed
  as reachy-mini-cli #183; a runtime restart restored it until the next
  lock. Harness workaround `9aab323`: `_tick_base_layer()` re-issues
  `feel-alive` as one bounded passive `run_behavior` (300 s) whenever the
  runtime's active list lacks it and nothing inhibits it, never under a held
  automatic lock, rate-limited, reset by every release
- `d9` — t7's cold-and-nameless refusal also refused the gaze stack's own
  ops: while people talked near the robot in Hebrew without naming it and
  nothing injected in between (12:28–12:36 BST), every `lock_face` and
  every feel-alive revive came back "not addressed", so the robot neither
  turned to the voices nor breathed. The gate exists for the model acting
  on overheard speech; the harness's reflexes are not the model. Fix
  `af9276c`: `IntentTools.execute(..., reflex=True)` skips only that check
  and `GazeStack._op` marks every op a reflex; the model's tool calls stay
  gated. The plan wrote t7 and t8/t9 for the same speech and the two met
  only live
...
| `t9` (`d8`) | the plan assumed the runtime's idle layer survives a lock; it does not (reachy-mini-cli #183), so the stack now revives it — a workaround that goes when the runtime re-seeds its own base layer | needs-follow-up |
| `t7` (`d9`) | the cold-and-nameless refusal was written for the model's tool calls but sat on the shared `IntentTools.execute` path the gaze stack also uses; a `reflex=True` keyword now separates the two | acceptable |
...
- tests: `uv run pytest -n auto` at `af9276c` — 2117 passed, 1 skipped (baseline before the round: 1810; after the plan's waves at `d5f044b`: 2041; after the review round at `6619e82`: 2070; after the live fixes at `61bde6f`: 2083; after #27 at `67d5d75`: 2096; after the revive at `9aab323`: 2114)
- tests: `tests/test_harness_boundary.py` — 19 passed at `8827b56` and `6619e82`
- tests: per-obligation node ids filed as plan evidence against obligations `o1`–`o28` (`devague evidence --list`), all proposed — `e1`–`e48`
...
- robot 11:51 local: `state.json` `active: []`, `ownership` all `None`, `intents.inhibitions: []`, daemon `control_mode: enabled`, head yaw −1° pitch 2° frozen across three reads; runtime journal 11:35:13 `lock_face ... 'inhibited': ['feel-alive', 'orient-to-sound']` → `set_inhibition event=feel-alive dropped reason=inhibited` → 11:35:31 `release_face ... 'inhibitions': []` and no `feel-alive` line after; runtime restart 11:53:01 → `active: [('feel-alive', base=True)]`, head wandering (`d8`, #183)
- robot 12:05:34–12:06:18 local (harness @ `9aab323`): base alive after the restart → no revive line; `reachy behavior stop feel-alive` at 12:06:10 → `[SENSE stage=gaze source=nova event=op] revive feel-alive ok=true` at 12:06:12 → runtime `run_behavior ... 'id': 'intent:run:feel-alive:2', 'class': 'passive'` → `active: [('feel-alive', False, 'intent:run:feel-alive:2', 297 s)]`, head yaw 4.5 → 1.7 → −0.2, pitch 7.2 → 12.3 → 3.5 over 4 s (`e46`)
- robot journal 12:28:34–12:36:45 local (harness @ `9aab323`, nameless Hebrew conversation at the desk): `lock face attempt=1..7 ... ok=false error=not addressed — the robot was not spoken to by name` on every conversation entry and retry; `revive feel-alive ok=false error=not addressed` every 30 s; `active` without `feel-alive`. After `af9276c` (restart 12:43:29): `revive feel-alive ok=true` at 12:43:36; nameless transcript at 12:43:43 → `layer wander -> conversation` → `lock face attempt=1 ... error=no face known` (the detector, not the gate); `active: [intent:run:feel-alive:4]`, head yaw 1.0 → 8.1 (`e48`)
- robot 13:27–13:31 local (`af9276c`): camera dead again since ≤ 12:43:35 — 239/239 snapshots without a frame at a free-running 48 Hz tick, every `look_at_face`/`lock_face` refused `no face known` with Ori in view ("It says it can't see anyone in front of it"), daemon journal silent, `/api/media/status` `available: false, released: true`, user journals rotated after ~35 min (root 91 % full); runtime restart 13:29:51 → `restored after=2783.81s`, 104/104 frames; an unneeded daemon restart followed (my script misread the JSON); the runtime's held client logged `GStreamer ... Internal data stream error` → `media released back to the daemon` → `media acquired` in 5 s; 102/102 frames at 20 Hz, face recognised (`e49`, reachy-mini-cli #176 comment). Runtime CPU ≈ 77 % of a core: GStreamer receive threads ≈ 36 points decoding 10 fps for a 1 fps detector; snapshots published per tick (20–48 Hz) — decimation proposed on #179
...
| effectful tools are refused while cold and nameless — for the model only; the body's reflexes pass | high | `tests/test_harness_tools.py` cold-refusal and reflex tests · live 12:43:43: a nameless sentence's lock reached the runtime · `e47`, `e48` |
...
| the robot stays alive after a lock evicts the runtime's idle layer, addressed or not | high | `tests/test_harness_gaze_stack_liveness.py` (15 tests) · live 12:06:10–12:06:18: evicted by name, revived in 2 s · live 12:43:36: revived while cold and nameless · `e45`, `e46`, `e48` |
...
| the harness names a dead camera and its return | high | journal `dropped reason=no-frames after=60s` at 09:32:15, `restored after=251.41s` at 10:47:54, the same pair across the runtime restart at 11:13:39 / 11:16:46, and `restored after=2783.81s` at 13:29:59 after the second silent death · `tests/test_harness_eyes.py` |
...
- the runtime's "nova" name — shipped as reachy-mini-cli #182 (v0.53.0 on main); once the device branch `wireless-motor-enable` carries it and the robot runs it, this harness writes `names = ["nova"]` into the overlay's operator head and submits a reload at start (same mechanics as the face-nod tombstone), behind `NOVA_ATTENTION_GATE`; c16 is delivered when that round-trips on the robot
